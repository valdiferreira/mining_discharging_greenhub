    ...
    # model = "95_percent_models_full/A0001.csv"
        ...
        if len(dataframe.index) > (len(dataframe.columns)-1)*10:
            ...
            if len(X_train.index) > (len(X_train.columns))*10:
                ...
                rgr = RandomForestRegressor(**ast.literal_eval(modelo["regressor"]))
                ...
                #pd.Series(y_pred).to_csv("random_forest_all/"+model.split("/")[1].split(".")[0]+"_y_pred.csv", sep=";")
                #pd.Series(y_test).to_csv("random_forest_all/"+model.split("/")[1].split(".")[0]+"_y_test.csv", sep=";")
                ...
                with open("random_forest_all/"+stats["model"]+".pkl", 'wb') as f:
...
if __name__ == "__main__":
    ...
    df_tunning_list=pd.read_csv("new_random_forest_top_100_sample.csv", sep=";")
    #--- 437.41035890579224 seconds ---
    ...
    # #para não sobrescrever os resultados anteriores
    shap_df=pd.concat(shap_df)
    pred_df=pd.DataFrame(pred_df)
    
    pred_df.to_excel("random_forest_all/ram_top_100_samples_random_forest_model_pred_df.xlsx")
    pred_df.to_csv("random_forest_all/ram_top_100_samples_random_forest_model_pred_df.csv", sep=";")
    shap_df.to_csv("random_forest_all/ram_top_100_samples_random_forest_model_shap_df.csv", sep=";")
    shap_df.to_excel("random_forest_all/ram_top_100_samples_random_forest_model_shap_df.xlsx")
